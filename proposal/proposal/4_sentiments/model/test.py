...
for entry in test_data:
    ...
    # Extract sentiment for OAs
    oas_with_sentiment = []
    for aspect, opinion in oas:
        sentiment = sentiment_pipeline(opinion)[0]  # Get sentiment prediction
        oas_with_sentiment.append((aspect, opinion, sentiment))

    # Extract sentiment for ISs
    iss_with_sentiment = []
    for sentence in iss:
        sentiment = sentiment_pipeline(sentence)[0]  # Get sentiment prediction
        iss_with_sentiment.append({"text": sentence, "sentiment": sentiment})
