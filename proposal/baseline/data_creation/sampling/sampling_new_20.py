...
def sample_oas_and_iss(summary, candidate_reviews, iss_data, sample_sizes, embeddings, embedding_matrix):
    ...
    # === STEP 1: Giữ lại các OAs gốc trong summary
    sampled_popular = summary["opinion_aspect_pairs"].copy()

    # === STEP 2: Lấy thêm 2 OAs gần nhất mỗi aspect
    for aspect in summary_aspects:
        original_opinions = [op for asp, op in summary["opinion_aspect_pairs"] if asp == aspect]
        ...
        # Sort theo similarity giảm dần và lấy 2 cao nhất
    ...
    # === STEP 4: Lấy ISs theo ROUGE similarity
...
if __name__ == "__main__":
    ...
    oas_file = "../extraction/results/extracted_OAs_1M_random.json"
    iss_file = "../extraction/results/extracted_ISs_1M_random.json"
    output_file = "results/sampling_keep_oa/mix_structured_data_1M_random_20.json"
    
    embeddings, embedding_matrix = load_glove_embeddings(glove_file)
    
    with open(oas_file, "r", encoding="utf-8") as f:
        oas_data = json.load(f)[950000:1000000]
